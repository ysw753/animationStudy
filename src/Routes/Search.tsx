...
import { IGetSearchMovie, searchMovies } from "../api";
...
function Search() {
  ...
  console.log(data)
  const onBoxClicked = (movieId:number)=>(history.push(`/search/${movieId}`));
  ...
  const movies = data?.results.filter((movie)=>movie.media_type==="movie")
  const tvs = data?.results.filter((tv)=>tv.media_type==="tv")
  const [mLeaving,setMLeaving]=useState(false);
  const [tLeaving,setTLeaving]=useState(false);
  const [mIndex, setMIndex]=useState(0)
  const [tIndex, setTIndex]=useState(0)
  const { scrollY } = useViewportScroll();
  const toggleMLeaving=()=>setMLeaving(prev=>!prev);
  ...
  const increaseMovieIndex=()=>{
    if(movies){
      ...
      if(totalMovies>6){
        if(mLeaving) return;
          toggleMLeaving();
          setMIndex((prev)=>prev===maxIndex ? 0 : prev+1)
      }
  ...
  return(
    <Wrapper>
      ...
      >
        ...
        >
          MOVIE</h1>
              <AnimatePresence initial={false} onExitComplete={toggleMLeaving}>
                <Row
                ...
                >
                  {movies
                    ?.slice(offset * mIndex, offset * mIndex + offset)
                    .map((movie) => (
                      <Box          
                        layoutId={movie.id+''} 
